...
const MobileChatView = ({ data }) => {
  ...
  const startFlow = (configItem) => {
    ...
    setStepIndex(0);
    // Use the Type from the sheet if available (in/out)
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl font-black text-xs uppercase transition-all active:scale-95 flex items-center justify-center gap-2
                      ...
                      : (type === 'out' ? 'bg-rose-500/10 border border-rose-500/20 text-rose-400' : 'bg-indigo-500 text-white shadow-xl shadow-indigo-500/20 col-span-2')}`}
                >
